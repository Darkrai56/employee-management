...
public class EmployeeController {
    ...
    public ResponseEntity<?> getEmployeesByDesignation(@PathVariable String designation) {
        try {
            List<EmployeeModel> employeeList = employeeService.getEmployeesByDesignation(designation);
            return new ResponseEntity<>(employeeList, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> getEmployeesByStatus(@PathVariable Boolean isActive) {
        try {
            List<EmployeeModel> employeeList = employeeService.getEmployeesByStatus(isActive);
            return new ResponseEntity<>(employeeList, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> getEmployeeByEmail(@PathVariable String email) {
        try {
            EmployeeModel employeeModel = employeeService.getEmployeeByEmail(email);
            return new ResponseEntity<>(employeeModel, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<?> searchEmployeeByName(@PathVariable String name) {
        try {
            List<EmployeeModel> employeeList = employeeService.searchEmployeeByName(name);
            return new ResponseEntity<>(employeeList, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
