...
public class LeaveController {
    ...
    public ResponseEntity<?> updateLeave(@PathVariable Integer id, @RequestParam String leaveType, @RequestParam String fromDate, @RequestParam String toDate, @RequestParam String reason, @RequestParam String status) {
        try {
    ...
    public ResponseEntity<?> getLeavesByStatus(@PathVariable String status) {
        try {
            List<LeaveModel> leaveList = leaveService.getLeavesByStatus(status);
            return new ResponseEntity<>(leaveList, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<?> getLeavesByEmployee(@PathVariable Integer employeeId) {
        try {
            List<LeaveModel> leaveList = leaveService.getLeavesByEmployee(employeeId);
            return new ResponseEntity<>(leaveList, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
